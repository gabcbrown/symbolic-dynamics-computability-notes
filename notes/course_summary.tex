...
\begin{fact}
  $\mu_F(E)$ is a probability measure on $E$.
\end{fact}
...
Let $\cN_{E^\infty, \sigma_E} = \{\nu \in \cM_{E^\infty, \sigma_E} : \int \sigma_E(x) d\nu(x) < \infty \}$, that is, the measures where the integral over the first return map is finite. Then we define the \textbf{lift} $l(\nu) = \frac{\nu}{\nu(x)}$, which gives us a natural bijection,
